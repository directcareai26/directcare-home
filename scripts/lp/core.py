...
(function(){{
  ...
  if (quiz) {{
    ...
    function render(){{
      ...
      qEl.classList.remove('step'); void qEl.offsetWidth; qEl.classList.add('step');
      oEl.classList.remove('step'); void oEl.offsetWidth; oEl.classList.add('step');
  ...
  // ends up below the fold of the box — you have to scroll INSIDE the iframe to
  // reach it, which on a phone is close to invisible.
  window.addEventListener('message', function(ev){{
    if (ev.origin !== 'https://business.tellescope.com') return;
    var d = ev.data;
    if (typeof d === 'string') {{
      if (d.toLowerCase().indexOf('submit') > -1) dl('intake_lead', {{form:'6a91c05fa9cb082abb340d5e'}});
    ...
    if (d.type === 'submit' || d.submitted) dl('intake_lead', {{form:'6a91c05fa9cb082abb340d5e'}});
  }});
}})();
</script>"""
